...
const Home = () => {
  ...
  useEffect(() => {
    // Cargar las tarjetas favoritas desde el almacenamiento local al cargar la página
    const storedFavorites = JSON.parse(localStorage.getItem("favoriteCards")) || [];
    setFavoriteCards(storedFavorites);
    // Resto de tu lógica de carga de tarjetas
  }, []);
  ...
  const handleFavoriteClick = async (cardId) => {
    ...
    if (!token) {
      console.error("Token no válido o no presente");
          // Lógica para marcar/desmarcar tarjetas como favoritas
    if (favoriteCards.includes(cardId)) {
      ...
      const updatedFavorites = [...favoriteCards, cardId];
      setFavoriteCards(updatedFavorites);
      // Actualizar el almacenamiento local
      localStorage.setItem("favoriteCards", JSON.stringify(updatedFavorites));
    }
